...
//wade(hotend_mount=wildseyed_mount, legacy_mount=false);
wade(hotend_mount=hotend_mount, legacy_mount=false);
...
////////// RENDER IDLER /////////////////////////////////////////////////////////////////////
translate([80+extra_gear_separation,-15,16.20])		// POSITIONING
	rotate([0,-90,0])
		wadeidler();
	...
	difference (){
		union(){
			...
			translate(idler_fulcrum){
				...
				difference(){
					...
					translate([idler_hinge_r+1,0,layer_thickness*4])
					cube([idler_hinge_r+5,
						idler_hinge_r*2+4+5,
						wade_block_depth/2-idler_short_side/2+
						idler_hinge_width+0.25+layer_thickness+10]);
				}
				rotate(-15)
		...
		rotate([-90,0,0]){
			...
			if (in_mask(hotend_mount,wildseyed_mount))
				wildseyed_mount_holes(insulator_d=12);
